namespace HumanDatabase
{
    public class TreeMemoryNodeManager<K, V> : ITreeNodeManager<K, V>
    {
        ...
        public TreeMemoryNodeManager(ushort minEntriesCountPerNode, IComparer<K> keyComparer)
        {
            ...
            entryComparer = Comparer<Tuple<K, V>>.Create((t1, t2) =>
            {
        ...
        {
            var newNode = new TreeNode<K, V>(this, (uint)(idCounter++), 0, entries, childrenIds);
        ...
        {
            var newNode = Create([new Tuple<K, V>(key, value)], [leftNodeId, rightNodeId]);
            rootNode = newNode;
            return newNode;
        }
