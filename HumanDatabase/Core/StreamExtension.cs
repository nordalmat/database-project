namespace HumanDatabase
{
    public static class StreamExtension
    {
        ...
        public static async Task<int> ReadAsync(this Stream src, byte[] buffer)
        {
            ...
            while (filled < buffer.Length)
            {
                lastRead = await src.ReadAsync(buffer.AsMemory(filled, buffer.Length - filled));
                filled += lastRead;
                if (lastRead == 0) break;
            }
        ...
        /// </summary>
        public static void CopyTo(this Stream src, Stream destination, int bufferSize = 4096, Func<long, bool>? feedback = null, long maxLength = 0)
        {
        ...
        /// </summary>
        public static async Task CopyToAsync(this Stream src, Stream destination, int bufferSize = 4096, Func<long, bool>? feedback = null, long maxLength = 0)
        {
            var buffer = new byte[bufferSize];
            var totalRead = 0L;
            ...
            {
                var bytesToRead = (int)Math.Min(maxLength - totalRead, buffer.Length);
                var thisRead = await src.ReadAsync(buffer.AsMemory(0, bytesToRead));
